...
const { DEFAULT_MKDOCS_YML } = require('../lib/constants');
...
async function renderMkdocsYAML (config, paths, outputPath) {
  const tmpl = path.join(paths.templates, 'mkdocs/mkdocs.yml.tmpl');
  const ymlTemplate = fs.pathExistsSync(tmpl) ? tmpl : path.join(__dirname, '..', DEFAULT_MKDOCS_YML);
