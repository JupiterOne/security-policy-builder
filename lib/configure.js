...
async function promptForValues (config) {
  ...
  const answers = await safeInquirerPrompt(missingOrEmptyOrganizationValues(config.organization));
  ...
  if (!program.noninteractive) {
    ...
    if (save.selected) {
      try {
        ...
        console.log(chalk.yellow(`Unable to save configuration to ${save.path}: ${err}`));
      }
    }
  }
...
async function promptForSecurityScorecardValues (config) {
  ...
  if (config.haveSecurityScorecard === true) {
    const toAsk = missingOrEmptyOrganizationValues(config, securityScorecardQuestions);
...
function missingOrEmptyOrganizationValues (org, values = questions) {
  ...
  return values.filter(q => { return (missingValues.includes(q.name) || org[q.name] === ''); });
}
...
function getAdoptedPartials (config) {
  ...
  for (const policy of config['policies']) {
    ...
    for (const id of policy.procedures || []) {
      ...
      if (!policyImplementor) {
        throw new Error(`reference to unknown adopted procedure/reference '${id}' in policy '${policy.id}'`);
      }
      if (!policyImplementor.adopted) { continue; }
      partials.push(policyImplementor);
    }
  }

  for (const ref of config['references']) {
    if (!ref.adopted) { continue; }
    ref.type = 'references';
    partials.push(ref);
  }
  return partials;
...
function getAdoptedProceduresForPolicy (policyId, config) {
  ...
  if (!policy) { throw new Error(`unknown policy id ${policyId}`); }
  let policyImplementor;
  for (const id of policy.procedures || []) {
    policyImplementor = config['procedures'].find(p => p.id === id);
    if (!policyImplementor) {
    ...
    if (!policyImplementor) {
      throw new Error(`reference to unknown procedure '${id}' in policy '${policy.id}'`);
    }
    if (!policyImplementor.adopted) { continue; }
...
function getAdoptedPSPElements (config) {
  ...
  for (const element of Object.keys(config)) {
    if (element === 'organization') { continue; }
