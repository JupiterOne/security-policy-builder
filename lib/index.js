...
module.exports = async function policybuilder (configuration, paths = defaultPaths) {
  ...
  await render.renderMkdocsYAML(configuration, paths, path.join(paths.output, 'mkdocs.yml'));
  await render.renderIndexPage(configuration, paths, path.join(paths.output, 'index.md'));
