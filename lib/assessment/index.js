...
function checkPrivSecOfficers(values, emptyValues) {
  if (
    emptyValues.filter((e) => e.match(/privacy|securityOfficerName/)).length > 0
  ) {
    let note =
...
function generateStandardControlsLookup(standardName, controlsMapping) {
  ...
  controlsMapping.procedures.forEach((control) => {
    const implementsStandard = control.implements
    ...
    if (implementsStandard.requirements) {
      implementsStandard.requirements.forEach((r) => {
        lookup[r] = lookup[r] || [];
        lookup[r].push(control.id);
      });
    }
    if (implementsStandard.controls) {
      implementsStandard.controls.forEach((c) => {
        lookup[c] = lookup[c] || [];
        lookup[c].push(c.id);
      });
    }
