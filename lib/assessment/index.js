...
function generateStandardControlsLookup (standardName, controlsMapping) {
  ...
  controlsMapping.procedures.forEach(control => {
    const implementsStandard = control.implements.filter(i => i.standard === standardName).pop();
...
async function calculateCPGaps (standardName, config) {
  const standardsDataDir = path.join(__dirname, '../../',
    DEFAULT_TEMPLATES,
    'standards');
...
function controlAdoptionFilter (cpIds, config) {
  ...
  cpIds.forEach(cpId => {
    ...
    if (!configuredCP) {
      console.log(`Control specified in controls-mapping not found in config.json: ${cpId}`);
    } else {
