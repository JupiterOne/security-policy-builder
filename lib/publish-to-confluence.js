...
async function publish(source, options) {
  ...
  const { domain, space, username, password } = options || await gatherCreds();
  ...
  console.log('Please retain `confluence-pages.json` in the current directory. The file contains Confluence page id mappings used for subsequent updates.');
  if (failed.length > 0) {
