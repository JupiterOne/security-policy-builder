...
async function main(config) {
  try {
    ...
    console.log(
      `Generating ${standard.toUpperCase()} self-assessment report...`
    );
...
function calculateInputGaps(inputs) {
  const gaps = [];
  Object.keys(inputs)
    .filter((i) => i.match(/has.*Gap/))
    .forEach((gapBooleanKey) => {
...
async function gatherInputs(organizationVars) {
  ...
  const answers = await configure.safeInquirerPrompt(
    assessment.questions(program.standard)
  );
  ...
  answers.date = moment();
  answers.isHIPAACoveredEntityText = answers.isHIPAACoveredEntity
    ? "is"
    : "is not";
  answers.isHIPAABusinessAssociateText = answers.isHIPAABusinessAssociate
    ? "is"
    : "is not";
