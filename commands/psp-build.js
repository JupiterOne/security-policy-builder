...
program
  ...
  .option('-n, --noninteractive', 'exit with error if any configuration data is missing (do not prompt)')
...
if (missing.length !== 0 && program.noninteractive) {
  error.fatal(`missing the following configuration value(s): ${missing.toString()}`, EUSAGEERROR);
}
...
async function main (config) {
  try {
    ...
    const { renderedPartials, renderedPSPDocs } = await policybuilder(config, paths);
...
function showStatus (items) {
  ...
  console.log(color(`${numOk}/${numTotal} ${pluralize(type, numOk)} processed OK`));
  if (numErrors > 0) {
