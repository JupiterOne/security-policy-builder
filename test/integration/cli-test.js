...
test.cb("psp-builder shows module version when run with --version", (t) => {
  execFile(cli, ["build", "--version"], { cwd: t.context.workDir }, function (
    err,
    stdout,
    stderr
  ) {
...
test.cb(
  "psp-builder errors if --noninteractive is given with insufficient --config JSON",
  (t) => {
      ...
      function (err, stdout, stderr) {
        if (!err) {
          t.fail(
            "non-zero rc",
            "rc 0",
            `Expected to fail with: ${cli} -s -c ${jsonFile}`
          );
        } else {
          t.pass();
        }
...
test.cb("psp-builder fails when unable to write to output dir", (t) => {
  ...
  execFile(
    ...
    function (err, stdout, stderr) {
      if (!err) {
        t.fail(
          `Expected to fail with: ${cli} --config ${jsonFile} -o /dev/null`
        );
      } else {
        t.pass();
      }
...
test.cb("psp-builder exposes templates dir on first use", (t) => {
  ...
  if (fs.pathExistsSync(exposedDir)) {
    t.fail(
      "Unexpected presence of templates dir prior to first use of psp builder."
    );
    t.end();
  }
