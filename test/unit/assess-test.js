...
test('assessment.validateOrgValues returns true when org values are populated', t => {
  ...
  t.is(
    true,
    assessment.validateOrgValues(org)
  );
});
...
test('assessment.questions returns parsed inquirer structure when valid standard is given', t => {
  t.truthy(
    assessment.questions('HIPAA')[0].message
  );
});
...
test('assessment.generateReport creates a reportfile', async t => {
  const outputDir = path.join(__dirname, '../fixtures', 'test_' + Math.random().toString(36).substring(2, 5));
...
test('assessment.generateGapSummary yields positive report when there are no gaps', t => {
  ...
  t.truthy(
    assessment.generateGapSummary([], config, 'HIPAA').match(/^.*met or exceeded all requirements.*$/)
  );
  t.truthy(
    assessment.generateGapSummary(['gap'], config, 'HIPAA').match(/^.*has compliance gaps.*$/)
  );
});
...
test('assessment.generateGapList yields one line of markdown per gap', t => {
  ...
  t.is(
    true,
    assessment.generateGapList(gaps).match(/\n/g).length === 2
  );
});
...
test('assessment.generateStandardControlsMapping shows Gaps', async t => {
  ...
  const mapping = assessment.generateStandardControlsMapping(annotatedRefs, config);
  t.truthy(
