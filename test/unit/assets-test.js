...
test.beforeEach(t => {
  ...
  t.context.scope = nock(server)
    .get('/notafile.png')
    .reply(404);

  t.context.outDir = 'test/fixtures/downloads/' + Math.random().toString(36).substring(7);
  t.context.logoURL = `${server}/${t.context.logoFile}`;
  t.context.badLogoURL = `${server}/notafile.png`;
  t.context.outFile = path.join(t.context.outDir, 'assets/images', t.context.logoFile);
  t.context.tests = [];
});
...
test('assets.downloadCustomLogo downloads and saves file', async t => {
  ...
  t.is(
    true,
    Object.keys(config).length < Object.keys(newConfig).length
  );
  const stat = require('util').promisify(fs.stat);
  t.is(
    true,
    (await stat(t.context.outFile)).isFile()
  );
});
