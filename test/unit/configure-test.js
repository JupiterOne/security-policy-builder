...
test("inquiries validates email addresses", (t) => {
  t.not(true, validate.email("not-an-address"));
  t.is(true, validate.email("me@privacy.net"));
});

test("inquiries validates web urls", (t) => {
  t.not(true, validate.webURL("git://foo/**/../@"));
  t.is(true, validate.webURL("http://bit.ly"));
});
